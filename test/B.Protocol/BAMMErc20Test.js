...
const LUSDToken = artifacts.require("VSTToken")
...
contract('BAMM', async accounts => {
  const [owner,
    ...
    A, B, C, D, E, F,
    u1, u2, u3, u4, u5,
    v1, v2, v3, v4, v5, frontEnd_2, frontEnd_3,
    bammOwner
  ] = accounts;
  ...
  describe("BAMMErc20", async () => {
    ...
    beforeEach(async () => {
      ...
      lqtyToken = VSTAContracts.vstaToken
      ...
      const frontEnd_1 = feePool

      let bammDeployer = bammOwner
      //bammDeployer = "0x1BE6064cA70e40A39473372bE0ac8a5e16F7Be45"

      const ETH = "0x0000000000000000000000000000000000000000"
      bamm = await BAMM.new(chainlink.address, stabilityPool.address, lusdToken.address, lqtyToken.address, erc20.address, 400, feePool, frontEnd_1, {from: bammDeployer})
      lens = await BLens.new()

      //console.log("bamm", bamm.address, "lens", lens.address)

      //await openTrove({ extraVSTAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: "0x1BE6064cA70e40A39473372bE0ac8a5e16F7Be45" } })      
    })
    ...
    it("deposit(): increases the Stability Pool LUSD balance", async () => {
      ...
      const stabilityPool_LUSD_After = await stabilityPool.getCompoundedVSTDeposit(bamm.address)
    ...
    it("deposit(): two users deposit, check their share", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })      
    ...
    it('rebalance scenario', async () => {
      ...
      const SPLUSD_Before = await stabilityPool.getTotalVSTDeposits()
      ...
      const SPLUSD_After = await stabilityPool.getTotalVSTDeposits()
      assert.isTrue(SPLUSD_After.lt(SPLUSD_Before))

      console.log((await stabilityPool.getCompoundedVSTDeposit(bamm.address)).toString())
      console.log((await stabilityPool.getDepositorVSTAGain(bamm.address)).toString())
      const price = await priceFeed.fetchPrice.call(erc20.address)
      ...
      const rate = await bamm.getConversionRate(lusdToken.address, erc20.address, toBN(dec(1, 18)), 0)
      ...
      await bamm.trade(lusdToken.address, toBN(dec(1, 18)), erc20.address, dest, rate, true, { from: alice });

      const swapBalance = await erc20.balanceOf(dest)

      assert.equal(swapBalance.toString(), ammExpectedEth.ethAmount.toString())
    })
    ...
    it("test basic LQTY allocation", async () => {
      ...
      await erc20.transfer(bamm.address, toBN(dec(3, 18)), {from: whale})
    ...
    it("test share + LQTY fuzzy", async () => {
      ...
      for(let i = 0 ; i < ammUsers.length ; i++) {
        await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: ammUsers[i] } })
        await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: nonAmmUsers[i] } })
    ...
    it("test complex LQTY allocation", async () => {
      ...
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      ...
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it('test share with ether', async () => {
      ...
      // Whale opens Trove and deposits to SP
      await openTrove({ asset: erc20.address, assetSent: dec(50, 'ether'), extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: A } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(20000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: B } })
      ...
      // 4k liquidations
      assert.equal(toBN(dec(6000, 18)).toString(), (await stabilityPool.getCompoundedVSTDeposit(bamm.address)).toString())
      const ethGains = web3.utils.toBN("39799999999999999975")
      //console.log(ethGains.toString(), (await stabilityPool.getDepositorVSTAGain(bamm.address)).toString())

      // send some ETH to simulate partial rebalance
      await erc20.transfer(bamm.address, toBN(dec(1,18)), {from: whale})
    ...
    it('price exceed max dicount and/or eth balance', async () => {
      ...
      assert.equal(priceDepletedWithFee.ethAmount.toString(), ethGains.toString())      
    })
    ...
    it('test getSwapEthAmount', async () => {
      ...
      assert.equal(priceWithFee.ethAmount.toString(), expectedReturn.mul(toBN(100)).div(toBN(100 * 105)).toString())
      assert.equal(priceWithFee.feeLusdAmount.toString(), dec(105, 18 - 2).toString())           
    })    
    ...
    it('test swap', async () => {
      ...
      // check lusd balance
      // should have 6105 lusd minus 1.05 that were taken for fees = 6103.95 = dec(610395, 16)
      assert.equal(toBN(dec(610395, 16)).toString(), (await stabilityPool.getCompoundedVSTDeposit(bamm.address)).toString())

      // check eth balance
      assert.equal((await erc20.balanceOf(dest)).toString(), priceWithFee.ethAmount.toString())
    ...
    it('test set params happy path', async () => {
      ...
      assert.equal(priceWithFee.feeLusdAmount.toString(), toBN(lusdQty.toString()).div(toBN(100)).toString())      
    })    
    ...
    it.skip('transfer happy test', async () => { // transfer is not supported anymore
      ...
      // Whale opens Trove and deposits to SP
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: A } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: C } })
      await openTrove({ asset: erc20.address, extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: D } })            
