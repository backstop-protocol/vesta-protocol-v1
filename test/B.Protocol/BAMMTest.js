...
contract('BAMM', async accounts => {
  ...
  describe("BAMM", async () => {
    ...
    async function deployToShuel() {
      const ERC20Test = artifacts.require("./ERC20Test.sol")
      console.log("deploying lens")

      const testnetLens = await BLens.new()
      console.log("lens", testnetLens.address)

      console.log("deploying token 1")
      const token1 = await ERC20Test.new()
      console.log("token1", token1.address)

      console.log("deploying token 2")
      const token2 = await ERC20Test.new()
      console.log("token2", token2.address)

      // tokens, bamms, lens
      
        /*
        Arg [0] : _priceAggregator (address): 0x94133ed86447733cb962320d7629d3d42bf59eb3
        Arg [1] : _SP (address): 0x52e5c875ce5897d5668fe833907ec50a5b050b9d
        Arg [2] : _LUSD (address): 0x38c9919e5f6349ba43681fe566031d9ee0ea3932
        Arg [3] : _LQTY (address): 0xa2f0cc74dfb91c57a21d65bfdde344f21037a532
        Arg [4] : _maxDiscount (uint256): 400
        Arg [5] : _feePool (address): 0x1000000000000000000000000000000000000001
        Arg [6] : _fronEndTag (address): 0x1000000000000000000000000000000000000001
  
        const bamm2 = await BAMM.new("0x94133eD86447733cB962320D7629d3d42BF59eB3",
                                      "0x52e5c875Ce5897d5668FE833907ec50a5B050b9d",
                                      "0x38C9919E5f6349bA43681fE566031d9ee0Ea3932",
                                      "0xA2f0CC74DFB91C57A21D65bfddE344f21037A532",
                                      400,
                                      "0x1000000000000000000000000000000000000001",
                                      "0x1000000000000000000000000000000000000001")
  
                                      console.log(bamm2.address)
  
        process.exit(1)*/

        // TODO non 18 decimals collateral price feed
        // TODO safe transfer tokens
    }
    ...
    beforeEach(async () => {
      ...
      bamm = await BAMM.new(chainlink.address, stabilityPool.address, lusdToken.address, lqtyToken.address, ETH, 400, feePool, frontEnd_1, {from: bammDeployer})
    ...
    it("deposit(): two users deposit, check their share", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraVSTAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraVSTAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })      
    ...
    it('rebalance scenario', async () => {
      ...
      const price = await priceFeed.fetchPrice.call(ZERO_ADDRESS)
    ...
    it("test share + LQTY fuzzy", async () => {
      ...
      for(let i = 0 ; i < ammUsers.length ; i++) {
        await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: ammUsers[i] } })
        await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: nonAmmUsers[i] } })
    ...
    it("test complex LQTY allocation", async () => {
      ...
      await openTrove({ extraVSTAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      ...
      await openTrove({ extraVSTAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraVSTAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it('test share with ether', async () => {
      ...
      // Whale opens Trove and deposits to SP
      await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: A } })
      await openTrove({ extraVSTAmount: toBN(dec(20000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: B } })
    ...
    it.skip('transfer happy test', async () => { // transfer is not supported anymore
      ...
      await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: A } })
      await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: C } })
      await openTrove({ extraVSTAmount: toBN(dec(10000, 18)), ICR: toBN(dec(20, 18)), extraParams: { from: D } })            
